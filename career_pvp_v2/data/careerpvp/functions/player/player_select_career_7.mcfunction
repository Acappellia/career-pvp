...
execute as @p[distance=..20,tag=choose_7 ] at @s unless score @s career matches 7 run execute as @e[tag=team] if score @s team = @p team if score @s career_dps matches 2.. run tellraw @p "该类别职业已经达到上限。如需和队友更换，请先取消职业选择"
execute as @p[distance=..20,tag=choose_7 ] at @s unless score @s career matches 7 run execute as @e[tag=team] if score @s team = @p team if score @s career_dps matches 2.. run tag @p remove choose_7
