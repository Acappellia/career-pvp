...
execute as @p[distance=..20,tag=choose_1 ] at @s run execute as @e[tag=team] if score @s team = @p team if entity @s[tag=chosen_1] run tellraw @p "不可以选择与队友重复的职业"
execute as @p[distance=..20,tag=choose_1 ] at @s run execute as @e[tag=team] if score @s team = @p team if entity @s[tag=chosen_1] run tag @p remove choose_1
