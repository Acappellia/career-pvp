...
execute as @p[distance=..20,tag=choose_3 ] at @s unless score @s career matches 3 run execute as @e[tag=team] if score @s team = @p team if score @s career_tank matches 1.. run tellraw @p "该类别职业已经达到上限。如需和队友更换，请先取消职业选择"
execute as @p[distance=..20,tag=choose_3 ] at @s unless score @s career matches 3 run execute as @e[tag=team] if score @s team = @p team if score @s career_tank matches 1.. run tag @p remove choose_3
