...
execute if score Team.13 member matches ..4 run tellraw @p[distance=..3,team=] "您已经成功加入队伍 13"
execute if score Team.13 member matches ..4 run tellraw @p[distance=..3,team=!,team=!13] "您已经成功更换至队伍 13"
execute if score Team.13 member matches ..4 run execute as @p[distance=..3,team=!13] at @s run function careerpvp:team/leave_team
