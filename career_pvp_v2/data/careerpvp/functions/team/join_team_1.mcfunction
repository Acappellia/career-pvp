...
execute if score Team.1 member matches ..4 run tellraw @p[distance=..3,team=] "您已经成功加入队伍 1 "
execute if score Team.1 member matches ..4 run tellraw @p[distance=..3,team=!,team=!1] "您已经成功更换至队伍 1 "
execute if score Team.1 member matches ..4 run execute as @p[distance=..3,team=!1] at @s run function careerpvp:team/leave_team
